...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/controller/manager"
)
...
func RestartPanel(core, agent, reload bool) {
	...
	if reload {
		if err := client.Reload(); err != nil {
			global.LOG.Errorf("restart HiTechCloud service failed, err: %v", err)
			return
		}
	}
	if core {
		if err := client.Operate("restart", "HiTechCloud-core"); err != nil {
			global.LOG.Errorf("restart HiTechCloud core service failed, err: %v", err)
			return
		}
	}
	if agent {
		if err := client.Operate("restart", "HiTechCloud-agent"); err != nil {
			global.LOG.Errorf("restart HiTechCloud agent service failed, err: %v", err)
			return
		}
	}
}
...
func loadFromPredefined(mgr Controller, keyword string) string {
	predefinedMap := map[string][]string{
		...
		"HiTechCloud-core":  {"HiTechCloud-core.service"},
		"HiTechCloud-agent": {"HiTechCloud-agent.service"},
