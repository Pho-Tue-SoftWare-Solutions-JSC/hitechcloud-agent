...
func (i *Iptables) ListPort() ([]FireInfo, error) {
	var datas []FireInfo
	basicRules, err := iptables.ReadFilterRulesByChain(iptables.ChainHiTechCloudBasic)
	if err != nil {
		return nil, err
	}
	beforeRules, _ := iptables.ReadFilterRulesByChain(iptables.ChainHiTechCloudBasicBefore)
...
func (i *Iptables) Port(port FireInfo, operation string) error {
	...
	if len(port.Chain) == 0 {
		port.Chain = iptables.ChainHiTechCloudBasic
	}
	...
	if port.Chain == iptables.ChainHiTechCloudBasicBefore {
...
func (i *Iptables) RichRules(rule FireInfo, operation string) error {
	...
	if len(rule.Chain) == 0 {
		rule.Chain = iptables.ChainHiTechCloudBasic
	}
	...
	name := iptables.BasicFileName
	if rule.Chain == iptables.ChainHiTechCloudBasicBefore {
		name = iptables.BasicBeforeFileName
	}
...
func EnableIptablesForward() error {
	...
	if err := iptables.AddChainWithAppend(iptables.NatTab, "PREROUTING", iptables.ChainHiTechCloudPreRouting); err != nil {
		return err
	}
	if err := iptables.AddChainWithAppend(iptables.NatTab, "POSTROUTING", iptables.ChainHiTechCloudPostRouting); err != nil {
		return err
	}
	if err := iptables.AddChainWithAppend(iptables.FilterTab, "FORWARD", iptables.ChainHiTechCloudForward); err != nil {
		return err
	}
...
func forwardPersistence() {
	if err := iptables.SaveRulesToFile(iptables.FilterTab, iptables.ChainHiTechCloudForward, iptables.ForwardFileName); err != nil {
		global.LOG.Errorf("persistence for %s failed, err: %v", iptables.ChainHiTechCloudForward, err)
	}
	if err := iptables.SaveRulesToFile(iptables.NatTab, iptables.ChainHiTechCloudPreRouting, iptables.ForwardFileName1); err != nil {
		global.LOG.Errorf("persistence for %s failed, err: %v", iptables.ChainHiTechCloudPreRouting, err)
	}
	if err := iptables.SaveRulesToFile(iptables.NatTab, iptables.ChainHiTechCloudPostRouting, iptables.ForwardFileName2); err != nil {
		global.LOG.Errorf("persistence for %s failed, err: %v", iptables.ChainHiTechCloudPostRouting, err)
	}
}

func iptablesListForward() ([]FireInfo, error) {
	natList, err := iptables.ListForward(iptables.ChainHiTechCloudPreRouting)
	if err != nil {
