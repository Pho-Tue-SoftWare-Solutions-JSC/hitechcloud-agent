...
func AddForward(protocol, srcPort, dest, destPort, iface string, save bool) error {
	...
	if dest != "" && dest != "127.0.0.1" && dest != "localhost" {
		iptablesArg := fmt.Sprintf("-A %s", ChainHiTechCloudPreRouting)
		if iface != "" {
		...
		if err := Run(NatTab, fmt.Sprintf("-A %s -d %s -p %s --dport %s -j MASQUERADE", ChainHiTechCloudPostRouting, dest, protocol, itemDstPort)); err != nil {
			return err
		}

		if err := Run(FilterTab, fmt.Sprintf("-A %s -d %s -p %s --dport %s -j ACCEPT", ChainHiTechCloudForward, dest, protocol, itemDstPort)); err != nil {
			return err
		}

		if err := Run(FilterTab, fmt.Sprintf("-A %s -s %s -p %s --sport %s -j ACCEPT", ChainHiTechCloudForward, dest, protocol, itemDstPort)); err != nil {
			return err
		}
	} else {
		iptablesArg := fmt.Sprintf("-A %s", ChainHiTechCloudPreRouting)
...
func DeleteForward(num string, protocol, srcPort, dest, destPort, iface string) error {
	itemDstPort := strings.ReplaceAll(destPort, "-", ":")
	if err := Run(NatTab, fmt.Sprintf("-D %s %s", ChainHiTechCloudPreRouting, num)); err != nil {
		return err
	}

	if dest != "" && dest != "127.0.0.1" && dest != "localhost" {
		if err := Run(NatTab, fmt.Sprintf("-D %s -d %s -p %s --dport %s -j MASQUERADE", ChainHiTechCloudPostRouting, dest, protocol, itemDstPort)); err != nil {
			return err
		}

		if err := Run(FilterTab, fmt.Sprintf("-D %s -d %s -p %s --dport %s -j ACCEPT", ChainHiTechCloudForward, dest, protocol, itemDstPort)); err != nil {
			return err
		}

		if err := Run(FilterTab, fmt.Sprintf("-D %s -s %s -p %s --sport %s -j ACCEPT", ChainHiTechCloudForward, dest, protocol, itemDstPort)); err != nil {
			return err
		}
	}
	return nil
...
func ListForward(chain ...string) ([]IptablesNatInfo, error) {
	if len(chain) == 0 {
		chain = append(chain, ChainHiTechCloudPreRouting)
	}
