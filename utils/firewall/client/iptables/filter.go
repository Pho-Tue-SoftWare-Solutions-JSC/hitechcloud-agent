...
func LoadInitStatus(clientName, tab string) (bool, bool) {
	...
	case "base":
		...
		initRules := []string{
			"-N " + ChainHiTechCloudBasicBefore,
			"-N " + ChainHiTechCloudBasic,
			"-N " + ChainHiTechCloudBasicAfter,
			fmt.Sprintf("-A %s %s -j ACCEPT", ChainHiTechCloudBasicBefore, strings.ReplaceAll(strings.ReplaceAll(IoRuleIn, "'", "\""), " -j ACCEPT", "")),
			fmt.Sprintf("-A %s %s -j ACCEPT", ChainHiTechCloudBasicBefore, strings.ReplaceAll(strings.ReplaceAll(EstablishedRule, "'", "\""), " -j ACCEPT", "")),
			fmt.Sprintf("-A %s %s", ChainHiTechCloudBasicAfter, DropAllTcp),
			fmt.Sprintf("-A %s %s", ChainHiTechCloudBasicAfter, DropAllUdp),
		}
		bindRules := []string{
			fmt.Sprintf("-A %s -j %s", ChainInput, ChainHiTechCloudBasicBefore),
			fmt.Sprintf("-A %s -j %s", ChainInput, ChainHiTechCloudBasic),
			fmt.Sprintf("-A %s -j %s", ChainInput, ChainHiTechCloudBasicAfter),
		}
	...
	case "advance":
		...
		lines := strings.Split(filterRules, "\n")
		initRules := []string{
			"-N " + ChainHiTechCloudInput,
			"-N " + ChainHiTechCloudOutput,
		}
		bindRules := []string{
			fmt.Sprintf("-A %s -j %s", ChainInput, ChainHiTechCloudInput),
			fmt.Sprintf("-A %s -j %s", ChainOutput, ChainHiTechCloudOutput),
		}
		return checkWithInitAndBind(initRules, bindRules, lines)
	case "forward":
		...
		initRules := []string{
			"-N " + ChainHiTechCloudPreRouting,
			"-N " + ChainHiTechCloudPostRouting,
		}
		bindRules := []string{
			fmt.Sprintf("-A PREROUTING -j %s", ChainHiTechCloudPreRouting),
			fmt.Sprintf("-A POSTROUTING -j %s", ChainHiTechCloudPostRouting),
		}
		...
		filterInitRules := []string{"-N " + ChainHiTechCloudForward}
		filterBindRules := []string{fmt.Sprintf("-A FORWARD -j %s", ChainHiTechCloudForward)}
...
func validateRuleSafety(rule FilterRules, chain string) error {
	...
	if chain == ChainInput || chain == ChainHiTechCloudInput || chain == ChainHiTechCloudBasic {
		if rule.SrcIP == "0.0.0.0/0" && len(rule.SrcPort) == 0 && len(rule.DstPort) == 0 {
			return fmt.Errorf("unsafe DROP is not allowed")
		}
	}

	if chain == ChainOutput || chain == ChainHiTechCloudOutput || chain == ChainHiTechCloudBasicAfter {
