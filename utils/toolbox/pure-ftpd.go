...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/toolbox/helper"
)
...
func NewFtpClient() (*Ftp, error) {
	...
	if err == nil {
		if err := cmd.RunDefaultBashCf("useradd -u 1000 -g %s %s", groupItem.Name, "HiTechCloud"); err != nil {
			return nil, err
		}
		return &Ftp{DefaultUser: "HiTechCloud", DefaultGroup: groupItem.Name}, nil
	}
	if err.Error() != user.UnknownGroupIdError("1000").Error() {
		return nil, err
	}
	if err := cmd.RunDefaultBashC("groupadd -g 1000 HiTechCloud"); err != nil {
		return nil, err
	}
	if err := cmd.RunDefaultBashC("useradd -u 1000 -g HiTechCloud HiTechCloud"); err != nil {
		return nil, err
	}
	return &Ftp{DefaultUser: "HiTechCloud", DefaultGroup: "HiTechCloud"}, nil
}
