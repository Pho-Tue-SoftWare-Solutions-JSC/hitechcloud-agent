...
func (t TaskRepo) UpdateRunningTaskToFailed() error {
	return getTaskDb(t.WithByStatus(constant.StatusExecuting)).Model(&model.Task{}).Updates(map[string]interface{}{"status": constant.StatusFailed, "error_msg": "HiTechCloud restart causes failure"}).Error
}
