...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/copier"
...
func (u *SnapshotService) SnapshotCreate(parentTask *task.Task, req dto.SnapshotCreate, jobID, retry uint) error {
	...
	if jobID == 0 {
		req.Name = fmt.Sprintf("HiTechCloud-%s-%s-linux-%s-%s", scope, versionItem.Value, loadOs(), time.Now().Format(constant.DateTimeSlimLayout))
	}
...
func snapBaseData(snap snapHelper, targetDir string, withDockerConf bool) error {
	...
	if global.IsMaster {
		err := snap.FileOp.CopyFile("/usr/local/bin/HiTechCloud-core", targetDir)
		snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-core"), err)
		if err != nil {
			return err
		}
	...
	err := snap.FileOp.CopyFile("/usr/local/bin/HiTechCloud-agent", targetDir)
	snap.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-agent"), err)
	if err != nil {
		return err
	}
	...
	svcAgentName, _ := controller.GetServicePath("HiTechCloud-agent")
...
func snapBackupData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	err := snap.FileOp.TarGzCompressPro(false, global.Dir.LocalBackupDir, path.Join(targetDir, "HiTechCloud_backup.tar.gz"), "", strings.Join(excludes, ","))
...
func snapPanelData(snap snapHelper, req dto.SnapshotCreate, targetDir string) error {
	...
	if !req.WithSystemLog {
		excludes = append(excludes, "./log/HiTechCloud*")
	}
	if !req.WithTaskLog {
	...
	excludes = append(excludes, req.IgnoreFiles...)
	err := snap.FileOp.TarGzCompressPro(false, rootDir, path.Join(targetDir, "HiTechCloud_data.tar.gz"), "", strings.Join(excludes, ","))
...
func loadPanelExcludes(req []dto.DataTree) []string {
	...
	for _, item := range req {
		if len(item.Children) == 0 {
			if !item.IsCheck {
				itemDir, _ := filepath.Rel(path.Join(global.Dir.BaseDir, "HiTechCloud"), item.Path)
				excludes = append(excludes, itemDir)
			}
		} else {
