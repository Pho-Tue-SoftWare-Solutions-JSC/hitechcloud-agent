...
func (u *RedisService) CheckHasCli() bool {
	...
	for _, item := range containerLists {
		if strings.ReplaceAll(item.Names[0], "/", "") == "HiTechCloud-redis-cli-tools" {
			return true
		}
	}
	return false
...
func (u *RedisService) InstallCli() error {
	item := dto.ContainerOperate{
		Name:     "HiTechCloud-redis-cli-tools",
		Image:    "redis:7.4.4",
		Networks: []dto.ContainerNetwork{{Network: "HiTechCloud-network"}},
	}
...
func confSet(redisName string, redisType string, updateType string, changeConf []redisConfig) error {
	...
	for i := 0; i < len(files); i++ {
		if files[i] == "# Redis configuration rewrite by HiTechCloud" {
			startIndex = i
			newFiles = append(newFiles, files[i])
			continue
		}
		if files[i] == "# End Redis configuration rewrite by HiTechCloud" {
			endIndex = i
			break
		}
	...
	if startIndex == 0 {
		newFiles = append(newFiles, "# Redis configuration rewrite by HiTechCloud")
	...
	newFiles = append(newFiles, "# End Redis configuration rewrite by HiTechCloud")
