...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/env"
...
func deleteAppInstall(deleteReq request.AppInstallDelete) error {
	...
	uninstall := func(t *task.Task) error {
		...
		uploadDir := path.Join(global.Dir.BaseDir, fmt.Sprintf("HiTechCloud/uploads/app/%s/%s", install.App.Key, install.Name))
...
func handleSSLConfig(appInstall *model.AppInstall, hasDefaultWebsite bool, sslRejectHandshake bool) error {
	...
	if !hasDefaultSSL {
		ca, _ := websiteCARepo.GetFirst(repo.WithByName("HiTechCloud"))
		if ca.ID == 0 {
