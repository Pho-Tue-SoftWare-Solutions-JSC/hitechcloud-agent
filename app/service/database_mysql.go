...
func (u *MysqlService) UpdateVariables(req dto.MysqlVariablesUpdate) error {
	...
	for _, info := range req.Variables {
		if info.Param == "slow_query_log" && info.Value == "ON" {
			logFilePath := filepath.Join(global.Dir.DataDir, fmt.Sprintf("apps/%s/%s/data/HiTechCloud-slow.log", app.Key, app.Name))
			if req.Type == "mariadb" {
				logFilePath = filepath.Join(global.Dir.DataDir, fmt.Sprintf("apps/%s/%s/db/data/HiTechCloud-slow.log", app.Key, app.Name))
			}
