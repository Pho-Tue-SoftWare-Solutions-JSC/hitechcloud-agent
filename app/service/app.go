...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/appicon"
...
func (a AppService) installWithHooks(req request.AppInstallCreate, executeScript bool, hooks *appInstallHooks) (appInstall *model.AppInstall, err error) {
	if err = docker.CreateDefaultDockerNetwork(); err != nil {
		err = buserr.WithDetail("ErrHiTechCloudNetworkFailed", err.Error(), nil)
		return
	}
	...
	if req.EditCompose {
		...
		if appDetail.DockerCompose == "" {
			dockerComposeUrl := fmt.Sprintf("%s/%s/HiTechCloud/%s/%s/docker-compose.yml", global.AppRepoURL(), global.CONF.Base.Mode, app.Key, appDetail.Version)
...
func (a AppService) GetAppUpdate() (*response.AppUpdateRes, error) {
	...
	versionUrl := fmt.Sprintf("%s/%s/HiTechCloud.json.version.txt", global.AppRepoURL(), global.CONF.Base.Mode)
...
func getAppList() (*dto.AppList, error) {
	list := &dto.AppList{}
	if err := getAppFromRepo(fmt.Sprintf("%s/%s/HiTechCloud.json.zip", global.AppRepoURL(), global.CONF.Base.Mode)); err != nil {
		return nil, err
	}
	listFile := filepath.Join(global.Dir.ResourceDir, "HiTechCloud.json")
