...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/alert_push"
...
func (u *CronjobService) removeExpiredLog(cronjob model.Cronjob) {
	...
	for i := int(cronjob.RetainCopies); i < len(records); i++ {
		...
		_ = os.Remove(pathUtils.Join(global.CONF.Base.InstallDir, "HiTechCloud/log/task/Cronjob", records[i].TaskID+".log"))
	}
}
