...
func (u *SnapshotService) SnapshotRollback(req dto.SnapshotRecover) error {
	...
	go func() {
		rootDir := fmt.Sprintf("%s/HiTechCloud_original/original_%s", global.Dir.BaseDir, snap.Name)
		...
		if global.IsMaster {
			taskItem.AddSubTask(
				i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-core"),
				func(t *task.Task) error {
					return FileOp.CopyFile(path.Join(baseDir, "HiTechCloud"), "/usr/local/bin")
				},
				nil,
			)
		}
		taskItem.AddSubTask(
			i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-agent"),
			func(t *task.Task) error {
				return FileOp.CopyFile(path.Join(baseDir, "HiTechCloud-agent"), "/usr/local/bin")
			},
			nil,
		)
		if global.IsMaster {
		...
		taskItem.AddSubTask(
			...
			func(t *task.Task) error {
				return FileOp.CopyDir(path.Join(rootDir, "HiTechCloud"), global.Dir.BaseDir)
			},
			nil,
		)
