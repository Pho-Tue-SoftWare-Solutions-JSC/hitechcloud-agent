...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/cloud_storage"
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/cloud_storage/client"
...
func (u *BackupService) checkBackupConn(backup *model.BackupAccount) (bool, error) {
	...
	fileItem := path.Join(global.Dir.BaseDir, "HiTechCloud/tmp/test/HiTechCloud")
	...
	_, _ = write.WriteString("HiTechCloud 备份账号测试文件。\n")
	_, _ = write.WriteString("HiTechCloud 備份賬號測試文件。\n")
	_, _ = write.WriteString("HiTechCloud Backs up account test files.\n")
	_, _ = write.WriteString("HiTechCloudアカウントのテストファイルをバックアップします。\n")
	write.Flush()

	targetPath := path.Join(backup.BackupPath, "test/HiTechCloud")
	...
	_, _ = client.Delete(path.Join(backup.BackupPath, "test/HiTechCloud"))
	return true, nil
}
