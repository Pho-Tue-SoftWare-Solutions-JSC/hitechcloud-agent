...
var (
	...
	svcCoreName, _     = controller.LoadServiceName("HiTechCloud-core")
	selCoreName, _     = controller.SelectInitScript("HiTechCloud-core")
	scriptCoreName, _  = controller.GetScriptName("HiTechCloud-core")
	svcAgentName, _    = controller.LoadServiceName("HiTechCloud-agent")
	selAgentName, _    = controller.SelectInitScript("HiTechCloud-agent")
	scriptAgentName, _ = controller.GetScriptName("HiTechCloud-agent")
)
	...
	go func() {
		...
		if req.IsNew || snap.InterruptStep == "RecoverBackups" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/HiTechCloud_backup.tar.gz"), snapJson.BackupDataDir, "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					return err
				},
				nil, 0, 90*time.Minute,
		...
		if req.IsNew || snap.InterruptStep == "RecoverPanelData" {
			taskItem.AddSubTaskWithAliasAndOps(
				...
				func(t *task.Task) error {
					...
					err := itemHelper.FileOp.TarGzExtractPro(path.Join(rootDir, snap.Name, "/HiTechCloud_data.tar.gz"), path.Join(snapJson.BaseDir, "HiTechCloud"), "")
					itemHelper.Task.LogWithStatus(i18n.GetMsgByKey("Decompress"), err)
					if err != nil {
						return err
					}
		...
		if req.IsNew || snap.InterruptStep == "RecoverCompose" {
			taskItem.AddSubTaskWithAliasAndOps(
				"RecoverCompose",
				func(t *task.Task) error {
					return restartCompose(path.Join(snapJson.BaseDir, "HiTechCloud/docker/compose"), &itemHelper)
				},
				nil, 0, 90*time.Minute,
			)
			req.IsNew = true
...
func backupBeforeRecover(name string, itemHelper *snapRecoverHelper) error {
	...
	rootDir := fmt.Sprintf("%s/HiTechCloud_original/original_%s", global.Dir.BaseDir, name)
	...
	if global.IsMaster {
		...
		err = itemHelper.FileOp.CopyFile("/usr/local/bin/HiTechCloud-core", baseDir)
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-core"), err)
		if err != nil {
			return err
		}
		svcCorePath, _ := controller.GetServicePath("HiTechCloud-core")
	...
	err = itemHelper.FileOp.CopyFile("/usr/local/bin/HiTechCloud-agent", baseDir)
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-agent"), err)
	if err != nil {
		return err
	}
	svcAgentPath, _ := controller.GetServicePath("HiTechCloud-agent")
...
func recoverBaseData(src string, itemHelper *snapRecoverHelper) error {
	...
	if global.IsMaster {
		...
		err = itemHelper.FileOp.CopyFile(path.Join(src, "HiTechCloud-core"), "/usr/local/bin")
		itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-core"), err)
		if err != nil {
			return err
		}
	...
	err := itemHelper.FileOp.CopyFile(path.Join(src, "HiTechCloud-agent"), "/usr/local/bin")
	itemHelper.Task.LogWithStatus(i18n.GetWithName("SnapCopy", "/usr/local/bin/HiTechCloud-agent"), err)
	if err != nil {
		return err
	}
