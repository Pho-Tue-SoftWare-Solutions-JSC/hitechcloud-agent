...
import (
	...
	fileUtils "github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/files"
...
const (
	rollbackPath = "HiTechCloud/tmp"
	upgradePath  = "HiTechCloud/tmp/upgrade"
	uploadPath   = "HiTechCloud/uploads"
	downloadPath = "HiTechCloud/download"
)
...
func (u *DeviceService) Scan() dto.CleanData {
	...
	originalPath := path.Join(global.Dir.BaseDir, "HiTechCloud_original")
	originalSize, _ := fileOp.GetDirSize(originalPath)
	treeData = append(treeData, dto.CleanTree{
		ID:          uuid.NewString(),
		Label:       "HiTechCloud_original",
		Size:        uint64(originalSize),
		IsCheck:     originalSize > 0,
		IsRecommend: true,
		Type:        "HiTechCloud_original",
		Children:    loadTreeWithDir(true, "HiTechCloud_original", originalPath, fileOp),
	})
...
func (u *DeviceService) Clean(req []dto.Clean) {
	...
	for _, item := range req {
		...
		case "HiTechCloud_original":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "HiTechCloud_original", item.Name))

		case "upgrade":
			dropFileOrDir(path.Join(global.Dir.BaseDir, upgradePath, item.Name))

		case "agent":
			dropFileOrDir(path.Join(global.Dir.BaseDir, "HiTechCloud/agent/package", item.Name))
		...
		case "system_log":
			if len(item.Name) == 0 {
				...
				for _, file := range files {
					if file.Name() == "HiTechCloud-Core.log" || file.Name() == "HiTechCloud.log" || file.IsDir() {
						continue
					}
...
func doSystemClean(taskItem *task.Task) func(t *task.Task) error {
	return func(t *task.Task) error {
		size := int64(0)
		fileCount := 0
		dropWithTask(path.Join(global.Dir.BaseDir, "HiTechCloud_original"), taskItem, &size, &fileCount)
		...
		if len(logFiles) != 0 {
			for i := 0; i < len(logFiles); i++ {
				...
				if logFiles[i].Name() != "HiTechCloud.log" && logFiles[i].Name() != "HiTechCloud-Core.log" {
...
func loadAgentPackage(fileOp fileUtils.FileOp) dto.CleanTree {
	pathItem := path.Join(global.Dir.BaseDir, "HiTechCloud/agent/package")
	...
	for _, file := range files {
		if file.IsDir() {
			...
			itemName := file.Name()
			isCurrentVersion := strings.HasPrefix(itemName, fmt.Sprintf("HiTechCloud-agent_%s_", global.CONF.Base.Version))
			if isCurrentVersion {
				continue
			}
...
func loadTreeWithAllFile(isCheck bool, originalPath, treeType, pathItem string, fileOp fileUtils.FileOp) []dto.CleanTree {
	...
	for _, file := range files {
		...
		if treeType == "system_log" && (file.Name() == "HiTechCloud-Core.log" || file.Name() == "HiTechCloud.log" || file.IsDir()) {
			continue
		}
...
func dropTaskLog(logDir string) {
	...
	case "Tamper":
		xpackDB, err := common.LoadDBConnByPathWithErr(path.Join(global.CONF.Base.InstallDir, "HiTechCloud/db/xpack.db"), "xpack.db")
		if err == nil {
