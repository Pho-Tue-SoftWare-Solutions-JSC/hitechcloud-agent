...
import (
	...
	"github.com/Pho-Tue-SoftWare-Solutions-JSC/hitechcloud-agent/utils/ssl"
...
func reloadSystemSSL(websiteSSL *model.WebsiteSSL, logger *log.Logger) {
	...
	if systemSSLEnable && sslID == websiteSSL.ID {
		...
		if err := fileOp.WriteFile(certPath, strings.NewReader(websiteSSL.Pem), 0600); err != nil {
			logger.Printf("Failed to update the SSL certificate File for HiTechCloud System domain [%s] , err:%s", websiteSSL.PrimaryDomain, err.Error())
			return
		}
		if err := fileOp.WriteFile(keyPath, strings.NewReader(websiteSSL.PrivateKey), 0600); err != nil {
			logger.Printf("Failed to update the SSL certificate for HiTechCloud System domain [%s] , err:%s", websiteSSL.PrimaryDomain, err.Error())
			return
		}
